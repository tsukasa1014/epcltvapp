...
## このforkについて
多量のルール、大量の録画ファイルがあるサーバー向けに以下の機能を実装しました
・検索ボタンの横に設定ボタンを追加
・ルールの並び順に「録画の新しい順」を追加
・「番組表からの録画」を追加
・追加リクエストの高速化
・一部のbug fix
テストは仮想マシンとFireTVでのみ行いました。
...
一度インストールしたあとは、アプリの中から更新できます。

1. メイン画面の「設定」行にある **アップデートを確認** を選ぶ。
2. GitHub へ接続してよいか確認が出るので **接続して確認** を選ぶ。
3. 新しいバージョンがあればリリースノートが表示される。**更新する** を選ぶとダウンロードが始まる。
4. ダウンロードが終わると端末のインストール確認画面が出るので、案内に従ってインストールする。

初回は「不明なアプリのインストール」の許可を求められることがあります。表示される案内から設定を開き、このアプリを許可してください。

GitHub へ接続するのは、この操作をして確認画面で承諾したときだけです。起動時やバックグラウンドで自動的に確認することはありません。
